        ...
        internal static void CompareProtocols(object currentDatagram, XElement layersContainer)
        {
            foreach (var layer in layersContainer.Protocols())
            {
                switch (layer.Name())
                {
                    ...
                    case "eth":
                        currentDatagram = new WiresharkDatagramComparerEthernet().Compare(layer, currentDatagram);
                        break;

                    case "arp":
                        currentDatagram = new WiresharkDatagramComparerArp().Compare(layer, currentDatagram);
                        break;

                    case "ip":
                        currentDatagram = new WiresharkDatagramComparerIpV4().Compare(layer, currentDatagram);
                        break;

                    case "igmp":
                        currentDatagram = new WiresharkDatagramComparerIgmp().Compare(layer, currentDatagram);
                        break;

                    case "icmp":
                        currentDatagram = new WiresharkDatagramComparerIcmp().Compare(layer, currentDatagram);
                        break;
